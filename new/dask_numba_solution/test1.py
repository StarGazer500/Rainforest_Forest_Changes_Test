...
# Calculate the common shape (intersection of both images)
...
# Load first image using dask from OpenCV
...
def process_array_in_blocks(dask_array, process_func, block_size=1024):
    """Process a dask array in blocks using the provided function"""
...
# Function to count pixels > 0
def count_positive_pixels(array):
...
gain_area = process_array_in_blocks(forest_gain, count_positive_pixels) * pixel_area / 10000  # hectares

print("Calculating forest loss area...")
loss_area = process_array_in_blocks(forest_loss, count_positive_pixels) * pixel_area / 10000  # hectares

print(f"Forest Gain: {gain_area:.2f} hectares")
print(f"Forest Loss: {loss_area:.2f} hectares")

# Create directory for output chunks if it doesn't exist
...
for i in range(0, common_height, chunk_size):
    ...
    for j in range(0, common_width, chunk_size):
        ...
        # Calculate the updated transform for this specific chunk
        ...
        # Convert from (height, width, bands) to (bands, height, width) for rasterio
...
# Ask if the user wants to create a mosaic from all chunks
...
if user_input.lower() == 'y':
    try:
        ...
        # Create a merged GeoTIFF using rasterio
        src_files_to_mosaic = []
        ...
        # Get the metadata from the first chunk
        ...
        output_mosaic = "forest_change_map_full.tif"
